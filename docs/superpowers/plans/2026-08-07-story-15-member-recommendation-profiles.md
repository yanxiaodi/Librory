...
- [ ] **Step 1: Write failing domain tests** for exclusion lists, preferred book languages, visibility/use flags, note length validation, explicit clearing, and normalization/deduplication.
- [ ] **Step 2: Run** `dotnet test tests/Librory.Domain.Tests/Librory.Domain.Tests.csproj --filter RecommendationProfile` and confirm the new tests fail because the fields and update semantics do not exist.
- [ ] **Step 3: Implement** the new fields, `ProfileVisibility` enum, bounded notes, and explicit update semantics without changing age-range ranking meaning.
- [ ] **Step 4: Run** the filtered domain tests and confirm they pass.
...
- [ ] **Step 1: Add failing DTO and model metadata assertions** for every new response field and the unique membership index.
- [ ] **Step 2: Run** the focused Application/API tests and confirm failure.
- [ ] **Step 3: Add EF configuration, migration, DTO fields, and response projection.** The migration must alter only `recommendation_profiles`; it must not modify family membership or account tables.
- [ ] **Step 4: Run** the focused tests and confirm persistence metadata and projection pass.
...
- [ ] **Step 1: Add failing API tests** covering owner create/update, admin update, normal-member denial of editing another member, family-visible read without notes, private read denial/limited projection, inactive/foreign target rejection, explicit clearing, and current-member alias behavior.
- [ ] **Step 2: Run** the focused API tests and confirm the new cases fail.
- [ ] **Step 3: Implement** route mapping, active-family membership lookup, permission checks based on database role/state, profile loading, partial update handling, and safe response shaping.
- [ ] **Step 4: Run** the focused API tests and confirm all permission and update cases pass.
...
- [ ] **Step 1: Add a failing member-list integration test** that creates a private/disabled profile and verifies the list exposes availability without leaking notes.
- [ ] **Step 2: Run** the focused API test and confirm failure.
- [ ] **Step 3: Implement** a profile-aware member projection scoped to the current family.
- [ ] **Step 4: Run** the focused API tests and confirm the target-selection shape is stable.
...
- [ ] **Step 1: Record** the permission model, explicit clearing behavior, and the boundary with Story 16/09 in the devlog.
- [ ] **Step 2: Run** all Domain, Application, and API tests.
- [ ] **Step 3: Run** `dotnet build` for the solution and `git diff --check`.
- [ ] **Step 4: Review** the complete diff for family isolation, private-note leakage, stale role claims, and accidental AI/scanning scope.
- [ ] **Step 5: Verify** `git status --short` contains only Story 15 files before committing and opening a PR.
