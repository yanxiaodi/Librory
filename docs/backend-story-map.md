...
`story-03` only creates `BookCopy` records. It assumes the book has already been resolved to a `BookEdition` by a separate catalog, metadata import, or recognition flow. If the edition is not known yet, the user must resolve that first rather than creating a provisional copy.
...
Version parsing and canonical edition import belong to later metadata work, not to the manual intake slice.

### Acceptance Criteria

- The API can create a persisted copy for a purchased book.
- The minimum intake path records the resolved edition, owning member, and duplicate confirmation state.
...
- As a family member, I want recommendation preferences to be stored once per member so the profile stays consistent.
...
This story should be implemented before `story-09` even though it has a higher ID. The numeric ID reflects when it was added to the map, not delivery order.

### User Stories

- As a developer, I want Librory data stored in PostgreSQL so it survives restarts.
- As a developer, I want EF Core migrations generated from a design-time DbContext so schema changes stay reproducible.
- As a developer, I want Scalar API docs with auth support so I can call protected endpoints while debugging locally.
- As a developer, I want the first API slice wired end-to-end so the application is more than domain-only behavior.

### Acceptance Criteria

- The API uses PostgreSQL through EF Core for application data.
- The solution includes a design-time DbContext factory suitable for `dotnet ef` commands.
- EF Core migrations are generated by tooling; migration files are not hand-authored.
- Entity configurations follow EF Core best practices with explicit keys, relationships, constraints, and indexes where appropriate.
- The API can start against a local PostgreSQL database with configuration from environment or user secrets.
...
- Keep the auth support scoped to developer debugging for now. If the login experience needs product-grade flows later, split that into a separate story.
- Keep migration files generated by EF Core commands only. Do not hand-edit migration history or schema SQL in this slice.
...
- As a developer, I want API integration tests to run against PostgreSQL so I can catch provider-specific bugs before merge.
- As a developer, I want the test harness to set up and tear down its own database so local verification stays simple.
- As a developer, I want migration and schema smoke checks in tests so the generated EF model stays trustworthy.

### Acceptance Criteria

- The API test project can boot against a real PostgreSQL instance.
- The test harness provisions an isolated database automatically for the test run.
- EF Core migrations are applied as part of the test fixture setup.
- At least one integration test proves the API works end-to-end against PostgreSQL.
- The test infrastructure remains runnable without manual database preparation.
