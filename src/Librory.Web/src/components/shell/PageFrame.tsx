...
export function PageFrame({ eyebrow, title, description, children }: PageFrameProps) {
  return (
    <section
      className={cn(
        'rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[var(--surface-elevated)] px-5 py-5 shadow-[var(--shadow-1)]',
        'sm:px-6 sm:py-6',
      )}
    >
      <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--text-tertiary)]">{eyebrow}</p>
      <h2 className="mt-2 font-[family-name:var(--font-display)] text-[1.55rem] font-normal italic tracking-[-0.01em] text-[var(--text-primary)]">
        {title}
      </h2>
